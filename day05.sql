...
-- 1) 매니저의 사번이 7698 이면, 직무는 'CLERK' 이거나 
...
SELECT ROUND(1234.46, 0)-- 1234
  FROM dual
;
--   ROUND(m) : n값을 생략하면 n = 0 인 것을 연산한 결과를 돌려줌
...
-----------------------TRUNC(m) : n이 생략된 경우는 n=0 으로 수행된다.
--                               즉, 소수점이하 모두 버림 연산으로 수행
SELECT TRUNC(1234.46) -- 1234
  FROM dual
;
